...
const canvasTxt = {
  ...
  drawText: function(ctx, mytext, x, y, width, height) {
    ...
    textarray.forEach((txtline,a) => {

      if(this.underline&&a==0){
        txtline = txtline.trim()
        ctx.fillText(txtline, textanchor, txtY)
        txtY += charHeight + fontSize/2
        const strokewidth = ctx.measureText(txtline).width
        ctx.strokeStyle = ctx.fillStyle;
        ctx.lineWidth = fontSize/10;
        ctx.beginPath()
        ctx.moveTo(x, y + charHeight + fontSize/4);
        ctx.lineTo(x + strokewidth, y + charHeight + fontSize/4);
        ctx.stroke();
      }else{
      txtline = txtline.trim()
      ctx.fillText(txtline, textanchor, txtY)
